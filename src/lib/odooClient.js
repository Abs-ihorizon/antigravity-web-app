class OdooClient {
  ...
  async rpcCall(endpoint, params) {
    ...
    try {
      ...
      // If it's literally a fetch network error, tag it explicitly so we can bounce to offline mode
  ...
  async fetchNextSequence(portalName) {
    ...
    try {
       ...
       // Intercept real mobile network packet crashes regardless of navigator.onLine!
       if (err.message === "OFFLINE_NETWORK" || err.message.includes("Failed to fetch")) {
           return { offline: true, error: null };
       }
